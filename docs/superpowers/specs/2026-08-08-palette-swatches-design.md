...
## Addendum (v1.4.4): index-stable overrides under same-k re-estimation

Pre-effect changes (levels especially) legitimately move palette colors beyond any fixed remap tolerance, which dropped overrides. But re-estimation from the same source under monotonic per-channel maps preserves luminance order, so when k is unchanged the luminance-sorted palette keeps its index semantics — overrides now stay at their index untouched (no distance check, no rerun). Nearest-color remapping applies only across k changes (color-count switches, auto-k flips), where index semantics genuinely break.
